...
int clearPathList[72 * pathNum] = {0};
float pathPenaltyList[72 * pathNum] = {0};
float clearPathPerGroupScore[72 * groupNum] = {0};
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        for (int i = 0; i < 72 * pathNum; i++) {
          clearPathList[i] = 0;
          pathPenaltyList[i] = 0;
        }
        for (int i = 0; i < 72 * groupNum; i++) {
        ...
        for (int i = 0; i < plannerCloudCropSize; i++) {
          ...
          if (dis < pathRange / pathScale && (dis <= (relativeGoalDis + goalClearRange) / pathScale || !pathCropByGoal) && checkObstacle) {
            for (int rotDir = 0; rotDir < 72; rotDir++) {
              float rotAng = (5.0 * rotDir - 180.0) * PI / 180;
              float angDiff = fabs(joyDir - (5.0 * rotDir - 180.0));
              if (angDiff > 180.0) {
                angDiff = 360.0 - angDiff;
              }
              if ((angDiff > dirThre && !dirToVehicle) || (fabs(5.0 * rotDir - 180.0) > dirThre && fabs(joyDir) <= 90.0 && dirToVehicle) ||
                  ((5.0 * rotDir > dirThre && 360.0 - 5.0 * rotDir > dirThre) && fabs(joyDir) > 90.0 && dirToVehicle)) {
                continue;
              }
        ...
        for (int i = 0; i < 72 * pathNum; i++) {
          ...
          if (clearPathList[i] < pointPerPathThre) {
            ...
            // float dirDiff = fabs(joyDir - endDirPathList[i % pathNum] - (5.0 * rotDir - 180.0));
            // if (dirDiff > 360.0) {
            //   dirDiff -= 360.0;
            // }
            // if (dirDiff > 180.0) {
            ...
            //zbh
            penaltyScore=1.0;
            // float dirDiff = fabs(joyDir - (5.0 * rotDir - 180.0));
            float dirDiff = fabs(joyDir - endDirPathList[i % pathNum] - (5.0 * rotDir - 180.0));

            // float score = (1 - sqrt(sqrt(dirWeight * dirDiff))) * rotDirW * rotDirW * rotDirW * rotDirW * penaltyScore;
            float score = (1000 - sqrt(dirWeight * dirDiff))+(7-abs(pathList[i % pathNum]-3))/100.0;
        ...
        if (selectedGroupID >= 0) {
          ...
          freePaths2.header.frame_id = "vehicle";//vehicle
